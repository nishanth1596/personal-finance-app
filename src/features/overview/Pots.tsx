...
const PotsItem = ({ title, value, color }: PotsItemProps) => {
  return (
    ...
    <article className="shadow-card rounded-xl bg-white p-6">
      ...
      <div className="grid grid-cols-1 pt-5">
        ...
        <div className="grid grid-cols-2 gap-y-4 pt-5">
          <PotsItem color="bg-green" title="Savings" value={159} />
          <PotsItem color="bg-cyan" title="Gift" value={40} />
          <PotsItem color="bg-navy" title="Concert Ticket" value={110} />
          <PotsItem color="bg-yellow" title="New Laptop" value={10} />
        </div>
      </div>
    </article>
  );
